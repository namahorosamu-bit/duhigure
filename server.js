...
// Twitter: simple user tweets (v2 recent tweets)
app.get('/api/twitter-feed', async (req, res) => {
  try {
    const username = req.query.username || 'your_twitter_username';
    // Lookup user id
    const u = await axios.get(`https://api.twitter.com/2/users/by/username/${encodeURIComponent(username)}`, {
      headers: { Authorization: `Bearer ${process.env.TWITTER_BEARER_TOKEN}` }
    });
    const uid = u.data.data.id;
    const tweets = await axios.get(`https://api.twitter.com/2/users/${uid}/tweets?max_results=5`, {
      headers: { Authorization: `Bearer ${process.env.TWITTER_BEARER_TOKEN}` }
    });
...
app.get('/api/facebook-feed', async (req, res) => {
  try {
    ...
    const r = await axios.get(`https://graph.facebook.com/${pageId}/posts?limit=5&access_token=${token}`);
    res.json(r.data);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
async function loadFeeds(){
  const tw = await fetch('/api/twitter-feed?username=@Sam_S54058').then(r=>r.json());
  const fb = await fetch('/api/facebook-feed').then(r=>r.json());
  // render into DOM elements you add, e.g. #twitter and #facebook
  document.getElementById('twitter').innerText = JSON.stringify(tw, null, 2);
  document.getElementById('facebook').innerText = JSON.stringify(fb, null, 2);
}
loadFeeds();
</script>